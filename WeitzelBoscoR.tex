...
Bosco \cite{weitzel2014accessing}
...
Bosco is designed to be run without administrator intervention.  It automatically installs the Bosco files   SSH was chosen as the protocol since it is used nearly universally for cluster access.  Jobs are submitted to Bosco, which then submits them over ssh to the remote cluster.  Input files are transferred over the SSH connection as well.  Bosco then monitors and reports the status of the job from idle on the remote cluster, running, and finally completed.  Once the job is completed, Bosco will transfer output files back to the submit host.
