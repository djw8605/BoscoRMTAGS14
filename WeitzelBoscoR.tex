...
GridR was modified to first create a submit file which will be submitted to the local system where Bosco is installed.  The submit file explicitly lists the input files and the expected output file.  The input files, as shown in Figure \ref{fig:gridrinput}, is the compiled function and input data.  Further, a separate script, the R bootstrap script, is sent to the worker node as well.  The bootstrap is further discussed in \ref{sec:bootstrap}.
